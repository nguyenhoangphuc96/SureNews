...
public class DetailInvestProjectActivity extends AppCompatActivity {
    Toolbar toolbar;
    ...
    private void addSamenews() {
        layoutSameNews = LayoutInflater.from(DetailInvestProjectActivity.this).inflate(R.layout.view_same_news, loBody, false);
    ...
    private void initData() {
        switch (id) {
            case 1: {
                listDetailHome1.add("http://www.baclieu.gov.vn/_layouts/LacVietBIO/fckUpload_BL/SiteChinh/2018-5/cac%20du%20an%20keu%20goi%20dau%20tu2852018_15306.jpg");
                listDetailHome1.add("Quang cảnh phiên họp thông qua dự thảo Quyết định ban hành Danh mục các dự án kêu gọi đầu tư xã hội hóa bức xúc để phát triển kinh tế - xã hội trên địa bàn tỉnh Bạc Liêu năm 2018 (Ảnh: Đức Hình)");
                listDetailHome1.add("Theo nội dung Quyết định, lĩnh vực giáo dục có 03 dự án kêu gọi đầu tư xã hội hóa bức xúc để phát triển kinh tế - xã hội trên địa bàn tỉnh là: Trường Mầm non và Tiểu học chất lượng cao, Trường Phổ thông liên cấp Song ngữ chất lượng cao và Trường Phổ thông liên cấp Quốc tế; lĩnh vực văn hóa có 03 dự án ưu tiên mời gọi đầu tư trong lĩnh vực dịch vụ - du lịch trên địa bàn tỉnh là: Xây dựng bến tàu du lịch Bạc Liêu - Côn Đảo và khu cảnh quan nhân tạo trên biển Bạc Liêu; điểm du lịch - dịch vụ Tắc Sậy và điểm du lịch Vườn chim Lập Điền.\n" +
                        "\n" +
                        "Căn cứ Danh mục các dự án kêu gọi đầu tư xã hội hóa, UBND tỉnh giao nhiệm vụ cho Sở Giáo dục và Đào tạo, Sở Văn hóa, Thể thao và Du lịch, UBND các huyện, thị xã, thành phố: Bạc Liêu, Giá Rai, Đông Hải xây dựng kế hoạch, chương trình thực hiện kêu gọi đầu tư các dự án do cơ quan mình quản lý và có trách nhiệm báo cáo UBND tỉnh; Sở Kế hoạch và Đầu tư chịu trách nhiệm chủ trì, phối hợp với các Sở, Ngành, địa phương liên quan công bố trên các phương tiện thông tin đại chúng và Cổng Thông tin điện tử tỉnh về Danh mục nêu trên; đồng thời tổng hợp, báo cáo kết quả thực hiện định kỳ 06 tháng và hàng năm về UBND tỉnh./.");
                listDetailHome1.add("L. Hiếu Gia");
                break;
            }
            case 2: {
    ...
    private void loadDataById(ArrayList<String> listDetailHome) {
        ...
        for(int i = 0; i<listDetailHome.size()-1;i++)
        {
            if (listDetailHome.get(i).startsWith("http")) {
                layoutImage = LayoutInflater.from(DetailInvestProjectActivity.this).inflate(R.layout.item_image, loBody, false);
                ...
                Picasso.with(DetailInvestProjectActivity.this).load(image).into(imgView);

                loBody.addView(layoutImage);
                i++;

            } else {
                layoutText = LayoutInflater.from(DetailInvestProjectActivity.this).inflate(R.layout.item_text, loBody, false);
        ...
        //author
        layoutAuthor = LayoutInflater.from(DetailInvestProjectActivity.this).inflate(R.layout.item_text, loBody, false);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.menu_speak: {
                ...
                if (!isClickSpeak) {
                    textToSpeech = new TextToSpeech(DetailInvestProjectActivity.this, new TextToSpeech.OnInitListener() {
                        @Override
